...
function UserManual() {
  return (
    <div className="p-4 bg-gray-100 rounded-lg shadow-md">
      <h2 className="text-2xl font-bold mb-4 font-simpson-title text-pink-simp">Instrucciones para Recorrer el Museo de Memes de los Simpsons</h2>
      <p className="text-2xl text-gray-700 mb-4 font-simpson-p">
      ...
      {instrucciones.map((item, index) => (
        <Disclosure key={index}>
          {({ open }) => (
            <>
              <Disclosure.Button className={`w-full px-4 py-2 text-left text-lg font-bold font-simpson-title text-gray-700 bg-blue-200 rounded-lg focus:outline-none focus-visible:ring focus-visible:ring-opacity-75 ${!open ? 'mb-4' : ''}`}>
                {item.titulo}
              </Disclosure.Button>
              <Disclosure.Panel className="px-4 pt-4 pb-2 font-simpson-p text-xl text-gray-700">
